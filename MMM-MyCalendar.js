...
Module.register("MMM-MyCalendar", {
	...
	getDom: function () {
		...
		for (var e in events) {
			var event = events[e];
			currentDate = moment(event.startDate, 'x');
			compareDate = moment(lastDate, 'x');
			
			if (!moment(compareDate).isSame(currentDate, 'day')) {
				lastDate = moment(event.startDate, 'x');
				var dateLineWrapper = document.createElement("tr");
				var dateWrapper = document.createElement("td");
				dateWrapper.className = "day bright";
				dateWrapper.colSpan = ((this.config.displaySymbol) ? 3 : 2);
				dateWrapper.innerHTML = moment(event.startDate, "x").format("dddd, DD. MMMM");
				dateLineWrapper.appendChild(dateWrapper);
				wrapper.appendChild(dateLineWrapper);
			}
...
//				if (event.startDate >= new Date()) {
					timeWrapper.innerHTML = moment(event.startDate, "x").format('HH:mm');
//				}
			}
			//timeWrapper.innerHTML += ' - '+ moment(event.startDate,'x').format('lll');
			//console.log(event);
			timeWrapper.className = "time light bright";
			eventWrapper.appendChild(timeWrapper);

			titleWrapper.innerHTML = "<div>" + this.titleTransform(event.title) + repeatingCountTitle + "</div>";
			if ( event.location ) {
				titleWrapper.innerHTML += "<div class='location'><span class='fa fa-map-marker'></span> " + event.location + "</div>";
			}
			titleWrapper.className = "title light bright";
			...
			if (this.config.fade && this.config.fadePoint < 1) {
				...
				if (e >= startingPoint) {
					...
					dateLineWrapper.style.opacity = 1 - (1 / steps * currentStep);
				}
			}
		}
